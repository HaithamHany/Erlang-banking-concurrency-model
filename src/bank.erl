    ...
    {loan_request, CustomerName, NeededLoanAmount} ->
      ...
      case BankResources - NeededLoanAmount >= 0 of
        true -> % Loan can be granted
          NewBankResources = BankResources - NeededLoanAmount, % Calculate the new bank resourceio:fwrite("[RESOURCE CALCULATION] Bank ~s granted loan to customer ~s of $~p  remaining resources of the bank are ~B dollars.~n", [BankName, CustomerName, NeededLoanAmount, NewBankResources]),
